...
import ProfileAlumniTable from './ProfileAlumniTable';
...
function getProducts() {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    products.push({
      ...
      name: 'name ' + id,
      surname: "surname " + id,
      role: Math.floor((Math.random() * 11)+1),
      phone: "084-656-"+ Math.floor((Math.random() * 1000) + 999),
      email: Math.floor((Math.random() * 20000) + 1)+"@gmail.com"
    });
  }
  return products;
}

export default class ProfileAlumniTableRec extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    return (
      <ProfileAlumniTable onSortChange={this.onSortChange.bind(this)}
